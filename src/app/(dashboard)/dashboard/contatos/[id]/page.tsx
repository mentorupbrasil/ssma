    ...
    <div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <p><span className="font-medium text-slate-700">Telefone:</span> {formatPhone(message.phone)}</p>
          {message.email && (
            <p><span className="font-medium text-slate-700">E-mail:</span> {message.email}</p>
          )}
          {message.company && (
            <p><span className="font-medium text-slate-700">Empresa:</span> {message.company}</p>
          )}
          <p><span className="font-medium text-slate-700">Assunto:</span> {message.subject}</p>
          <div className="rounded-lg border border-slate-100 bg-slate-50 p-4">
            <p className="whitespace-pre-wrap leading-relaxed text-slate-700">{message.message}</p>
          </div>
          <p className="text-xs text-slate-500">
            Recebido em {format(message.createdAt, "dd/MM/yyyy 'às' HH:mm")} · Origem: {message.source}
          </p>
        </CardContent>
      </Card>
    </div>
