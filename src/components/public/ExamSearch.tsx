...
export function ExamSearch({ exams }: { exams: Exam[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <p className="py-12 text-center text-slate-500">Nenhum exame encontrado.</p>
      ) : (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((exam) => (
            <ExamCard
              ...
              preparation={exam.preparation ?? "Consulte a clínica"}
              deliveryTime={exam.deliveryTime ?? "A definir"}
              notes={exam.notes}
            />
          ))}
        </div>
